...
Можно разбирать число с помощью ``union`` (см. ``print_union.c``). Обратите внимание, что хранятся bitfields начиная с самого младшего бита структуры. Нельзя объявлять переменные типа bitfield, брать от них адрес.
